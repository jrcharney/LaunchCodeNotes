...
I'm pretty sure we covered this back in Chapter 4 (4.4.2.3., to be exact), but it bears repeating.
...
As we noted earlier, for booean functions, it is conventional that their names start with `is` or `has` whenever possible.

Creating a ver/noun pair for a function name doesn't always make sense, but when it does, you should use this format to create a good descriptive name.

### 10.7.3. Use Descriptive Names

We have repeated reminded you to use descriptive names, but now we want to expand on this point. You should *prefer long, descrivtive names over short abbreviated names.* If you can read a function name and understand what it does from the name alone, then the function has a good name.
...
> convertC					// ❌
> // Though `convertCtoF` would be a good compromise.
> 
> isValidLaunchCode			// ✅
> validCode					// ❌
> 
...
If you find yourself writing a commen to describe what your function does, consider a better name that might remove the need for such additional explaination. The best function (and variable) names are those that are *self-documenting*---descriptive enough to not need furuthre explanation.

Using self-documenting names means that the code that *uses* your function will be more readable, since your explanatory comments are not visible when the function is used. Additionally, it's easy for comments to be inaccurate; when you update your code to change behavior, there's nothing forcing you to also update your commens. For this reason, some programmer live by the maxim "Comments lie". While we won't go as foar as to say you should never use comments in your code, we do believe that comments should not be used to make up for port function and variable names.

> :nerd_face: **Actually...** You should document your functions as much as possible. Try using [JSDoc](https://jsdoc.app/) style documentation, because sometimes things aren't exactly what they say on the tin.
>
> Also, if you live by "Comments lie", you should take the time to revise your comments often!

### 10.7.4. Check Your Understanding :white_check_mark:

> :question: **Question**: Which is the best name for the following function?
>
> ```js
...
In mathematics, there is a function called composition that describes building a new function from other functions.
$$
(f \circ g)(x) = f(g(x))
$$
In calculus, the derivative of a compositing function is known as the chain rule, as it defines the derivative of two functions chained together.
$$
(f \circ g)'(x) = f'(g(x)) \cdot g'(x) = ((f' \circ g) \cdot g')(x) = (f' \circ g \cdot g')(x)
$$
I know, too much math for the brain to handle. (If you think that's difficult, you should see the anti-derivative of such a function.) That's why we have computers!

Similarly, in programming, the practice of using functions to build other functions is known as **function composition** or simply **composition**. In this section, we'll explore this.

### 10.8.1. Palindrome Checker

<iframe width="560" height="315" src="https://www.youtube.com/embed/JUQDzj6R3p4" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

A palindrome can be defined as a word that is spelled backwards and forwards. Some examples are "tacocat" (though that is two words), "kayak", and "racecar".

> :information_source: **NOTE**: There are other factors that are sometimes included in the defintion of a palindrome. For example, an alternate defintion is that a palindrome is a sentence or phrase that contains letters in the same order (like in that Weird Al video I embedded above), whether considered from beginning-to-end, or end-to-beginning, ignoring punctuation, case and spaces.
...
function reverse(str) {
    let reversed = '';	// don't forget that d on the end!
    
    for (let i = 0; i < str.length; i++) {
        reversed = str[i] + reversed;
    }
    
    return reversed;	// again, don't forget the d!
}
```

This is the same algorithm that we used previously to ==reverse a string== back in Chapter 9.

> :reminder_ribbon: **TODO**: Insert a link to the section in the highlighted area above to the section where that happened. Ditto for the highlighted area in the next paragraph.

Another approach is to use the fact that there is a `reverse` method for arrays, and that the methods `split` and `join` allow us to go from strings to arrays and back. (This was covered back in ==Chapter 8==.) ([link](https://repl.it/@launchcode/reverse-Function), though I'm pretty sure we did this already.)

```js
function reverse(str) {
    // MOD: Just to make sure we don't put anything between '',
    // lets use a constant string in split and join
    const SEP = '';		
    let lettersArray = str.split(SEP);
    let reversedLettersArray = lettersArray.reverse();
    return reversedLettersArray.join(SEP);
}
```

Let's break down steps carried out by this function:

1. **Turn the string into an array of characters** with the String function `split` and `''` (empty string) as the splitting character to return an array of individual characters that make up the string.
2. **Reverse the array of characters** with the Array function `reverse`.
3. **Join the reversed character array into a string** using the Array function `join` and `''` to concatenate each individual character together into a single string.

> 🧩 **Try It!** Using method chaining to reduce the `reverse` function to a single line. ([Link](https://repl.it/@launchcode/isPalindrome))
...
Since `isPalindrome` uses our `reverse` function, we have demostrated composition.

`isPalindrome` was our $f(u)$ and `reverse` was our $u = g(x)$ which created $f(g(x))$ or $(f \circ g)(x)$.

### 10.8.2. Functions Should Do Exactly One Thing

> 💋 **K.I.S.S.**: Keep It Simple, Stupid!

An important consideration when writing a function is size. By "size" we mean that a function should be short, and more importantly, *do exactly one thing*.

This principle is eaiser to state than put into practice. For example, what if we had written `isPalendrome` without breaking out the `reverse` code into a separate function?
...
Some cases are will be more clear-cut, however.  Suppose we wnated to expand the capability of our program to do more than one thing. It wouldn't be in our best interest to do more than one thing, and we would need to describe our functions with longer, more complex names.
...
> :reminder_ribbon: **TODO**: I wish someone had made a nod to [currying](https://en.wikipedia.org/wiki/Currying) here.

Why is this better? For one thing, smaller functions are eaiser to debug. And by separating single responsibilites into individual functions, we also make our code easier to read and more reusable. In looking at the `makeLunch` function, it's very clear what is going oin. First it makes a sandwhich, then it pours a drink.

Were the `makeLunch` function to simply contain all of the code necessary to carry out *both* tasks, there would be no clear separation between one taks and the other, and the only way we might describe the various sections of the larger function would be to use comments. And as far as the people who wrote this course are concerned, they'd rather you use commens as a secondary option to explain your code.

## 10.9. Why Create Functions?

After wading through all of the new syntax necessary to create a function, you might be asking yourself, *Why would I never want to do this?*

> :nerd_face:  **Actually...** I was thinking more like "when is this blasted chapter ever going to end!" :tired_face:

These are good questions. So here are a few answers:

### 10.9.1. Functions Reduce Repetition

Like loops, functions help keep our code <abbr title="Don't Repeat Yourself">DRY</abbr> :cactus: When we need to repeat the same basic task in multiple parts of a program, a function allows us to package up that taks into a need, reusable form. Loops enable the same task to be repeated many times in succession, while functions enable the same task to be repeated in different portions of a program.

### 10.9.2. Functions Make Your Code More Readable

Placing a piece of functional code within a function allows us to put a name on that functionality. 

> :tired_face: I'm not going to repost the code! It's already in this document.

While the variable name `reversed` is descriptive, giving us a sense of what is going on the `for` loop, the function becomes more readable when we break out the reversing behavior in a separate function.

> :tired_face: Again, we wrote something eariler.

> :reminder_ribbon: **TODO**: At least point out where in those :tired_face: sections.

Aside from following the principle that functions should do only one thng, the logic within `isPalindrome` is more clear and self-descriptive. The function itself says, *a string is a palindrome if it is equal to its reverse*. To draw this same conclusion, from the example above, without a `reverse` function, we are required to analyse more of the program's logic.

### 10.9.3. Functions Reduce Complexity

Large programs can be brokend won into smaller parts using functions. Imagine a car built out of a single, large piece of metal. Were such a car to break down, diagnosing the problem would be difficult, and fixing it nearly impossible. (Sort of like most Apple products! :drum: [rimshot] ) The mechanic would have to determine where the issue was, then cut apart the bad portion, create a custom-made replacement portion, and weld it into place. (That can get expensive!)

The complexity of the situation becomes much less when a car is made of lots of small parts, each of which can be tested and replaced individually. The same thing happens with code. There are many other organizational units for programs--including modules, files, and packages--function are the most basic and universal organizational tool.

### 10.9.4. Functions Enable Code Sharing

Encapsulating behavior within a function makes it easy to reuse that code within a program, but it also allows you to share that behavior across files and even different projects. This becomes critically important when you start working on bigger program that consist of a large number of files.

We'll explore this idea in [Chapter 13](13.%20Modules.md) when we discuss Modules.

### 10.9.5. Functions Save Millions of Lives Every Day

> A little bit dramatic, isn't it?

You'e right, blockquote! We were being over dramatic. But the point is, function are incredibly powerful tools that you will come to appreciate and find indispensible. Ask a profession programmer if they could do their hob without functions, an the answer will be an emphatic "NO!"

While functions may see abstract and difficult ot learn at first, repeated practice will lead to mastery. WE promise that your work will be worth it!

## 10.10. Exercises: Functions :runner:

> :relieved: Oh thank goodness it's almost over!

> :reminder_ribbon: **TODO** and :nerd_face: **Actually...** I should still post links to the exercises that I did in this section. If you want, add the notes from [this section](https://education.launchcode.org/intro-to-professional-web-dev/chapters/functions/exercises.html) later.
...
:checkered_flag: We're a third of the way done with this book! Which this chapter was a big part of it. **You know what comes after Functions? [MORE FUNCTIONS](11.%20Functions%20(Part%202),md)!** ( :tired_face: )
