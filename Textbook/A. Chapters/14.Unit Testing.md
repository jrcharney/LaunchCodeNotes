...
> 5. ==Rerun the test and see the `it` fail.==
...
In many of your previous coding tasks, you had to verify that your code worked before moving to the next step. This often requires you to add `console.log` statements to to you ce to check the value stored in a variable or returned from a function. This approach finds and fixes syntax, reference, or logic errors *after* you write your code.

In this chapter, you learned how to use testing to solve coding errors. Even better, you learned how to *prevent* mistakes by wriing tests cases before completing the code. The exercises below offer practice with using test to find bugs, and the studio asks you to implement TDD.

### 14.6.1. Automatic Testing to Find Errors

Let's begin with the following simple code ([Link](https://repl.it/@launchcode/UnitTestingExercises02)):
...
The function checks to see if a number is greater than, less than, or equal to 5. We do not nreally need a function to do this, but it provides good pracitce for writing test cases.

Note that the REPL.it contains three files:

1. `checkFive.js` which holds the code for the function.
2. `spec/checkFive.spec.js`, which will hold the testing code.
3. `index.js` which holds special code to make Jasmine work.

> :warning: **WARNING!** Do not change the code in `index.js`. Messing with this file will disrupt the automatic testing.

Our tasks this time around:

1. We need to add a few lines to `checkFive.js` and `spec/checkFive.spec.js` to get them to talk to each other.

   1. `spec/checkFive.spec.js` needs to access `checkFive.js` and we also need ot import the `assert` testing function. Add two more `require` statements to accomplish this. (Review [14.3. Unit Testing In Action](##14.3.%20Unit%20Testing%20In%20Action) if nedded.) (We'll do this below, a couple steps from now.)

   2. Make the `checkFive` function available to the spec file, by using `module.exports`. (Again, seel 14.3.)
...
2. Set up your first test for the `checkFive` function. In the `spec/checkFive.spec.js` file, add a `describe` function with one `it` clause:

   ```js
   // @file: spec/checkFive.spec.js
...
3. Now write a test to see if `checkFive` produces the correct output when passed a number *less than 5*.
   ...
   1. First, replace `"Descriptive feedback..."` with a *detailed* message. This is the test the user will see if the test *fails*. Do NOT skimp on this! Refere back to Section 14.2.2.4., [Specifications and Assertions](####14.2.2.4.%20Specifications%20and%20Assertions) to review best practices.
   ...
   3. Now use `assert` to check the result:
   ...
   5. Now change line 5 in `checkFive.js` to `if (num > 5)` and rerun the test. The output should look similar to:
   ...
   6. Change line 5​ back.

      > :information_source: **NOTE**: We do NOTE need to check every possible value that is less than 5. Testing a single example is sufficient to chek that part of the function.

4. Add two more `it` clauses inside `describe`---one to test what happens when `checkFive` passes a value greater than 5, and the other to test whether the value equals 5.
   ...
   describe("checkFive", function(){
      ...
      it("Is equal to five", function(){
         let output = checkFive(5);
         assert.strictEqual(output, "5 is equal to 5.");
      });
   
      it("Greater than five", function(){
         let output = checkFive(7);
         assert.strictEqual(output, "7 is greater than 5.");
      });
   
   });
   ```

   > :reminder_ribbon: **TODO**: Insert Console output

### 14.6.2. Try One on Your Own

Time for Rock, Paper, Scissors! The function below takes the choices `rock`, `paper`, or `scissors` of two players as its parameters. It then decides which player won the match and returns a string. ([Link](https://repl.it/@launchcode/UnitTestingExercises03))
...
1. Set up the `RPS.js` and `spec/RPS.spec.js` files to talk to each other. (If you need help, re-read the previous exercise or Section 14.2. "Hello, Jasmine!")
2. Write a test in `spec/RPS.spec.js` to check if `whoWon` behaves correctly when the players tie (both choose the same option). Click "Run" and examine the output. (Spoiler alert: the code for checking ties is correct in `whoWon`, so the test should pass. If it doesn't, modify your `it` statement.)
3. Write tests (one at a time) for each of the remaining cases. Run the tests after each addition, and modify the code as needed. There is one mistake in `whoWon`. You might spot it on your own, but try to use autmoated testing to identify and fix it.

> :bulb: **Tip**: If writing tests one at a time seems tedious, and you know what you want to say ahead of time, you can always write it out, then comment out the other tests using multi-line commenting (`/* ... */`) and move the `/*` part after each success further down as you go along.

### 14.6.3. Bonus Mission :rocket:

![Image from https://puzzlewocky.com/](../../Images/RockPaperScissorsLizardSpock.jpg)

What if something OTHER than `rock`, `paper`, or `scissors` is passed into the `whoWon` function? ([What if someone adds `spock` and `lizard`?](https://puzzlewocky.com/parlor-games/rock-paper-scissors-lizard-spock/)) Modify the code to deal with the possibility.

Don't forget to add more `it` clause to `spec/RPS.spec.js` to test for these cases.

> :information_source: **NOTE**: This is just bonus mission. If anything, just add one extra case to the regular rock-paper-scissors rules to deal with that smart alec who wants to Spock his way out of RPS. 

> :nerd_face: **MATH NOTE**: There's a reason why you should just stick with RPS if you have two players. Adding more options, makes the game more complicated. But *RPSLS* is useful if there are **three** player. There's like a whole schpeal about combinatorics, permutations, and factorial that I could go into here, but you're probably not interested. I just thought I'd give it a quick shout out.
