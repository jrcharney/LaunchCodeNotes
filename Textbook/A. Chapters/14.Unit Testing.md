...
2. Set up your firt test for the `checkFive` function. In the `spec/checkFive.spec.js` file, add a `describe` function with one `it` clause:
...
1. Set up the `RPS.js` and `spec/RPS.spec.js` files to talk to each ohter. (If you need hep, re-read the previous exercise or Section 14.2. "Hello, Jasmine!")
